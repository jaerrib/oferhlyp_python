...
def turn_loop(my_game):
    my_game.board.display_board()
    print(my_game.jumped_list)
    ...
    if selected_token == "end turn":
        ...
        if (
            ...
            print(f"Possible moves/jumps are: {my_game.get_moves_list(possible_moves)}")
            entered_move = input(
            ...
            if move_position in possible_moves["possible_moves"]:
                my_game.board.data[move_row][move_col] = my_game.board.data[row][col]
                my_game.board.data[row][col] = 0
                my_game.turn_over = True
            if move_position in possible_moves["possible_jumps"]:
                ...
                print("In list?", jumped_token in my_game.jumped_list)

                if (
                    my_game.board.data[jumped_token[1]][jumped_token[0]].player
                    != my_game.active_player
                ) and jumped_token not in my_game.jumped_list:
                    my_game.board.data[move_row][move_col] = (my_game.board.data)[row][
                        col
                    ]
                    my_game.board.data[row][col] = 0
                    my_game.board.data[jumped_token[1]][jumped_token[0]].hp -= 1
                    my_game.jumped_list.append(
